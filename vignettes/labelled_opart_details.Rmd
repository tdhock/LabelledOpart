...
```{r}
sizes <- seq(100, 1000, 100)
timing_list <- list()
labels <- data.frame("start" = c(1, 10, 50, 90), "end" = c(5, 15, 55, 95), "breaks" = c(1, 1, 0, 0))
...
gg_runtime <- ggplot(data = timing.dt1, aes(x = size, y = time, col = expr)) +
geom_point() +
geom_smooth() +
...
```{r}
labels2 <- labels
labels2$change_points <- as.character(labels2$breaks)
#labels

ggplot() + geom_point(aes(position, signal), data = sel_data, shape = 1)+
  penaltyLearning::geom_tallrect(aes(xmin=starts,xmax=ends),
                                     data=labels,color="pink",fill="pink")+
